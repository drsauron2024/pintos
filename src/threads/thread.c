...
//a little helper function to check if a list_elem is in a list
bool
list_elem_is_in_list (struct list_elem *e)
{
  return e->next != NULL && e->prev != NULL;
}
//bool fraction for thread priority comparison
bool
...
//bool fraction for donation priority comparison
bool
...
//It may be a chain of donations
//Therefore, we need to implement a recursive donation function
void
...
void
remove_donations_by_lock(struct lock *lock){
  if(thread_mlfqs) return; //No priority donation in mlfqs mode
...
{
  if(thread_mlfqs) return; //No priority setting in mlfqs mode
  struct thread *current_thread = thread_current();
  ...
  current_thread->priority = effective_priority;
  if(!intr_context())
  {
    thread_yield();
  }
...
init_thread (struct thread *t, const char *name, int priority)
{
  ...
  //Initialize donation realated members
  t->priority = priority;
  ...
  t->priority = priority;
  t->wake_ticks = 0; //No need to sleep when initialized
